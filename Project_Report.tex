...
\begin{itemize}[leftmargin=*]
    \item \textbf{8 Classical Cipher Algorithms:} Caesar, Monoalphabetic, Vigenère, Playfair, Hill, Columnar Transposition, Differential XOR, and Base64
...
\subsubsection{Security Analysis}
\begin{itemize}
    \item \textbf{Vulnerabilities:} Kasiski examination, Index of Coincidence
    \item \textbf{Security Level:} \textcolor{orange}{Medium} - More secure than Caesar
    \item \textbf{Complexity:} O(n) - Linear time
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Advantages:} Digraph frequency different from single letter frequency
    \item \textbf{Vulnerabilities:} Digraph frequency analysis still possible
    \item \textbf{Security Level:} \textcolor{orange}{Medium}
...
\subsubsection{Security Analysis}
\begin{itemize}
    \item \textbf{Advantages:} Frequency analysis completely ineffective, diffusion property
    \item \textbf{Vulnerabilities:} Known-plaintext attack (requires $n^2$ plaintext-ciphertext pairs)
    \item \textbf{Security Level:} \textcolor{green}{High} (for classical standards)
    \item \textbf{Complexity:} O(n) with O($n^2$) matrix operations (constant size)
    \item \textbf{Historical Significance:} Predecessor of modern block ciphers
\end{itemize}
...
\subsubsection{Security Analysis}
\begin{itemize}
    \item \textbf{Advantages:} Very fast (XOR is hardware-native), diffusion property
    \item \textbf{Vulnerabilities:} Known-plaintext attack trivial, error propagation
    \item \textbf{Security Level:} \textcolor{orange}{Medium} - Not secure without proper key
    \item \textbf{Complexity:} O(n) - Linear time
    \item \textbf{Modern Equivalent:} Stream ciphers like ChaCha20
\end{itemize}

\subsection{Base64 Encoding}

\subsubsection{Description}
Base64 is \textbf{NOT an encryption algorithm} - it is an encoding scheme that converts binary data to ASCII text. It is included here as a utility tool. Base64 is widely used in email (MIME), URLs, and data embedding.
...
\subsubsection{Security Analysis}
\begin{itemize}
    \item \textbf{Warning:} \textcolor{red}{NOT ENCRYPTION!} - Provides zero security
    \item \textbf{Purpose:} Binary-to-text encoding for data transport
    \item \textbf{Reversibility:} Trivial decoding (no key required)
    \item \textbf{Overhead:} 33\% size increase
    \item \textbf{Use Cases:} Email attachments, JWT tokens, data URLs
\end{itemize}
